...
from pd_vtk import pv_read, pv_save, vtk_df_to_mesh, vtk_mesh_to_df, vtk_plot_meshes, vtk_Voxel, vtk_meshes_bb
...
class GridMine(object):
  ...
  def calc_mine(self):
    for mesh in self._m0:
      self.mine_mesh(mesh, False)
    
    if self.blank:
      self.fill(np.inf)

    for mesh in self._m1:
      self.mine_mesh(mesh, True)
